...
(function (root, factory) {
  ...
  class HttpPartSource {
    /* names: array of urlencoded-ready filenames under baseURL */
    ...
    async fetchRange(i, a, b) {
      const url = this.baseURL + encodeURIComponent(this.names[i]);
      const r = await fetch(url, { headers: { Range: "bytes=" + a + "-" + (b - 1) } });
      if (r.status !== 206 && r.status !== 200) throw new Error("HTTP " + r.status + " for part " + i);
